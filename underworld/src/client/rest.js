import { fetchJson, queryParameters } from 'admin-on-rest/lib/util/fetch';
import {
  CREATE, DELETE, GET_LIST, GET_MANY, GET_MANY_REFERENCE, GET_ONE,
  UPDATE,
  ...
  const convertRESTRequestToHTTP = (type, resource, params) => {
    ...
    switch (type) {
      case GET_LIST: {
        ...
        options.headers.set('Range', ((page - 1) * perPage) + '-' +
          ((page * perPage) - 1));
        const pf = params.filter;
        Object.keys(pf)
          .forEach(b => pf[b] = 'ilike.' + pf[b].replace(/:/, '') + '%',);
        let query = {
